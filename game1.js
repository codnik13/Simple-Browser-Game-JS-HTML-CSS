var runner, debris=[], debriswidth, debrisheight, runnerx, runnery, canvaswidth, canvasheight, gap, timeout;
function sizeset(){
  canvaswidth=window.innerWidth-2; canvasheight=Math.ceil((window.innerHeight-2)*0.925); runnerwidth=Math.ceil(canvaswidth/25);
  runnerheight=runnerwidth+10; debriswidth=debrisheight=Math.floor(canvaswidth/22); gap=Math.floor(canvasheight/30);
}
...
sizeset();
function prestart(){
  runner=new item(canvaswidth/2, canvasheight-gap-runnerheight, runnerwidth, runnerheight, '#FF0033');
  matrix.start();
}
function game(){ 
  matrix.welcome();
  timeout=setTimeout(prestart, 2000);
}
const matrix={
...
function rand(v){
  let piece=Math.floor((canvaswidth-runnerwidth))/11;
  if(v==1) return Math.floor(Math.random()*piece)+2*piece;
  if(v==2) return Math.floor(Math.random()*piece)+8*piece;
  if(v==3) return Math.floor(Math.random()*piece)+4*piece;
  if(v==4) return Math.floor(Math.random()*piece)+6*piece;
  if(v==5) return Math.floor(Math.random()*piece)+piece;
  if(v==6) return Math.floor(Math.random()*piece)+9*piece;
  if(v==7) return Math.floor(Math.random()*piece)+3*piece;
  if(v==8) return Math.floor(Math.random()*piece)+7*piece;
  if(v==9) return Math.floor(Math.random()*piece);
  if(v==10) return Math.floor(Math.random()*piece)+10*piece;
...
function reset(){
  ...
  for(i=0; i<debris.length; i++){
   ...
   if(++matrix.count%30==0){
     if(++matrix.eleven>11) matrix.eleven=1;
...
function stop(){
  ...
  debris.length=0; runner.x=canvaswidth/2; runner.y=canvasheight-gap-runnerheight; matrix.count=-1; matrix.stop=1;
}
function restart(){
...
matrix.timer.addEventListener('click', function(){
  if(matrix.pause){
    matrix.pause=false;
    debris.pop();
    matrix.interval=setInterval(reset,20);
    ...
    debris.push(new item(0, 0, matrix.canvas.width, matrix.canvas.height, 'rgba(100,100,100,0.5)'));
    debris[debris.length-1].falls();
    matrix.timer.innerHTML='continue';
    return;
})
